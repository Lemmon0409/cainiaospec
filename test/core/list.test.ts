...
describe('ListCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `cainiaospec-list-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should handle missing cainiaospec/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No CainiaoSpec changes directory found. Run 'cainiaospec init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'cainiaospec', 'changes');
